...
Worker.prototype.progress = function (id, completed) {
  var self = this;
  if (!self.active) return;
  if (completed === undefined) {
    ...
    Object.keys(self._waiting).forEach(function (id) {
      if (!self._waiting[id]) return;
      self.progress(id, completed*self.progress.tasks[id].total);
    })
  } else {
    // Apply to id
    self.progress.complete = 0;
    self.progress.tasks[id].complete = completed;
    self.progress.tasks[id].pct = Math.max(0, Math.min(1, completed/self.progress.tasks[id].total));
    self._taskIds.forEach(function (taskId, id) {
    ...
    self.emit('task_progress', id, completed);
  }
}
