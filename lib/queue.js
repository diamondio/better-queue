...
Queue.prototype._queueTask = function (taskId, newTask, ticket) {
  ...
  var putTask = function () {
    var writeId = uuid.v4();
    self._writing[taskId] = { id: writeId, task: newTask };
    self._store.putTask(taskId, newTask, priority, function (err) {
      if (err) return self._tickets[taskId].failed('failed_to_put_task');
      ...
      if (self._writing[taskId] && self._writing[taskId].id === writeId) {
        ...
        self.emit('task_queued', taskId, newTask);
      ...
      // Get the next batch. If already fetching, mark that there are additions to the queue
