...
Queue.prototype._pushTask = function (taskId, task, ticket) {
  ...
  var updateTask = function (isNew) {
    self.priority(task, function (err, priority) {
      ...
      self.store.putTask(taskId, task, function (err) {
        ...
        self.store.putPriority(taskId, priority, function (err) {
  ...
  self.store.getTask(taskId, function (err, oldTask) {
...
Queue.prototype._getNextBatch = function (cb) {
  this.store[this.filo ? 'getLastN' : 'getFirstN'](this.batchSize, cb)
}
