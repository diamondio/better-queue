...
Queue.prototype.use = function (store, opts) {
  ...
  if (typeof store === 'string' && stores[store]) {
    ...
    var Store;
    try {
      Store = require('better-queue-' + store.type);
    } catch (e) {
      throw new Error('Attempting to require better-queue-' + store.type + ', but failed.\nPlease ensure you have this store installed via npm install --save better-queue-' + store.type)
    }
