...
Queue.prototype.push = function (input, cb) {
  ...
  self.filter(input, function (err, task) {
    ...
    ticket.accept();
    self._pushTask(taskId, task, ticket);
  })
  return ticket;
}
...
Queue.prototype._deleteTask = function (taskId, cb) {
...
Queue.prototype._addToQueue = function (taskId, priority, cb) {
  var self = this;
...
Queue.prototype._getFirstN = function (n, cb) {
  cb(null, this._queue.splice(0, n));
}

Queue.prototype._getLastN = function (n, cb) {
  cb(null, this._queue.splice(-n).reverse());

}

Queue.prototype._pushTask = function (taskId, task, ticket) {
  var self = this;
  var updateTask = function (isNew) {
    self.priority(task, function (err, priority) {
      ...
      self._putTask(taskId, task, function (err) {
        ...
        self._addToQueue(taskId, priority, function (err) {
          if (err) return ticket.failed('failed_to_add_to_queue');
          ticket.queued();
          if (!self._timeout) {
...
Queue.prototype._getNextBatch = function (cb) {
  var self = this;
  var batch = {};
  self[self.filo ? '_getLastN' : '_getFirstN'](self.batchSize, function (err, taskIds) {
    if (err) return cb({ message: 'failed_to_get_next_batch' });
    async.map(taskIds, function (taskId, cb) {
      self._getTask(taskId, function (err, task) {
        if (err || task === undefined) return cb();
        batch[taskId] = task;
        self._deleteTask(taskId, function () {
          cb();
        });
      })
    }, function () {
