...
var takeNextN = function (first) {
  return function (n, cb) {
    ...
    var sql = function (fields) {
      return "SELECT " + fields + " FROM tasks WHERE lock = '' ORDER BY priority DESC, added " + (first ? "ASC" : "DESC") + util.format(' LIMIT %d', n);
    };
    var lockId = uuid.v4();
    self.Tasks.update({ lock: lockId, }, { where: [util.format('id IN (%s)', sql("id"))] }).then(function (results) {
