...
var takeNextN = function (first) {
  return function (n, cb) {
    ...
    var lockId = uuid.v4();
    self.sequelize.query(`UPDATE tasks SET lock = '${lockId}' WHERE id IN (` + sql('id') + ")", { type: self.sequelize.QueryTypes.UPDATE }).then(function (results) {
      // FIXME: check how many rows were modified
      cb(null, lockId);
    }).error(function (err) {
      cb(err);
    });
