...
function SqlStore(opts) {
  ...
  if (dialect === 'sqlite') this.adapter = (new require('./SqliteAdapter'))(opts);
  else if (dialect === 'postgres') this.adapter = (new require('./PostgresAdapter'))(opts);
...
var takeNextN = function (first) {
  return function (n, cb) {
    ...
    var lockId = uuid.v4();
    this.adapter.run(util.format("UPDATE %s SET lock = '%s' WHERE lock = '' AND id IN (" + sql(this.tableName, 'id', n) + ")", this.tableName, lockId), function (err, result) {
...
SqlStore.prototype.releaseLock = function (lockId, cb) {
  this.adapter.run(util.format("DELETE FROM %s WHERE lock = '%s'", this.tableName, lockId || ''), cb);
};
