...
var takeNextN = function (first) {
  return function (n, cb) {
    ...
    var lockId = uuid.v4();
    self.db.run("UPDATE tasks SET lock = ? WHERE id IN (" + sql('id') + ")", [lockId, n], function (err) {
...
SQLiteStore.prototype.connect = function (cb) {
  ...
  self.db = new sqlite3.Database(self.path, function (err) {
    ...
    self.db.run("PRAGMA journal_mode=WAL;", function (err) {
      if (err) return cb({ message: 'failed_to_set_journal_mode' });
...
SQLiteStore.prototype.putTask = function (taskId, task, priority, cb) {
  ...
  self.db.serialize(function () {
    self.db.run("BEGIN TRANSACTION", function () {
      self.db.run("INSERT OR REPLACE INTO tasks VALUES (?, ?, ?, ?, NULL)", [taskId, '', serializedTask, priority || 1], function () {
        self.db.run("COMMIT TRANSACTION", function () {
          cb();
        });
      });
    });
...
SQLiteStore.prototype.getLock = function (lockId, cb) {
  var self = this;
  self.db.all("SELECT id, task FROM tasks WHERE lock = ?", lockId, function (err, rows) {
    if (err) return cb(err)
    var tasks = {};
...
SQLiteStore.prototype.getRunningTasks = function (cb) {
  var self = this;
  self.db.all("SELECT id, task, lock FROM tasks", function (err, rows) {
    if (err) return cb({ message: 'failed_to_fetch_tasks' });
    var tasks = {};
    rows.forEach(function (row) {
...
SQLiteStore.prototype.releaseLock = function (lockId, cb) {
  var self = this;
  self.db.run("DELETE FROM tasks WHERE lock = ?", lockId, cb);
}
